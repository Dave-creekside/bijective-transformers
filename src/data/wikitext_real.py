...
# Import HuggingFace datasets library avoiding naming conflict
try:
    hf_datasets = importlib.import_module('datasets')
    load_dataset = hf_datasets.load_dataset
except ImportError:
    print("❌ HuggingFace datasets library not installed. Please install with: pip install datasets")
    sys.exit(1)
...
class WikiTextDataset(Dataset):
    ...
    def __init__(
        ...
        print(f"Dataset ready: {len(self.processed_data)} samples")
...
class WikiTextDataModule:
    ...
    def setup(self):
        """Setup all datasets."""
        print("Setting up WikiText-2 datasets...")
    ...
    def get_vocab_size(self) -> int:
        ...
        if self.train_dataset is None:
            ...
            temp_dataset = WikiTextDataset(
                split="validation",  # Use validation for speed
                tokenizer_name=self.tokenizer_name,
