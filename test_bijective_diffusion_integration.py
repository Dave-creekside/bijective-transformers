...
from src.data.corruption import TextCorruptor, CorruptionConfig
...
def test_training_step_with_corruption():
    ...
    # Create text corruptor
    corruption_config = CorruptionConfig(
        mask_prob=0.3,
        random_prob=0.1,
        keep_prob=0.6
    )
    corruptor = TextCorruptor(corruption_config, vocab_size=config.transformer.transformer.vocab_size)
    
    # Test data
    batch_size, seq_len = 2, 12
    clean_input_ids = torch.randint(1, config.transformer.transformer.vocab_size, (batch_size, seq_len))
    attention_mask = torch.ones(batch_size, seq_len)
...
def test_hybrid_configuration():
    ...
    config = create_bijective_diffusion_model_config(
        ...
        num_layers=4,
        num_heads=2,
        transformer_hybrid_layers=[0, 2]  # Only these layers are bijective
    )
...
def main():
    ...
    print("🚀 Bijective Discrete Diffusion Integration Tests")
    print("=" * 60)
