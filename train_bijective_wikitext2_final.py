#!/usr/bin/env python3
"""
FINAL: Training script for bijective discrete diffusion model on REAL WikiText-2.
FIXED: Generation display, validation tensor mismatch, and tokenizer warnings.
"""
...
def compute_perplexity(model, dataloader, device: str = "cpu") -> float:
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            # FIXED: Ensure consistent sequence length
            ...
            try:
                # FIXED: Use training_step instead of forward for consistency
                metrics = model.training_step(
                    clean_input_ids=input_ids,
                    attention_mask=attention_mask,
                    corruptor=model._temp_corruptor  # Use stored corruptor
                )
...
def train_bijective_model():
    ...
    print("🚀 Training Bijective Discrete Diffusion Model on REAL WikiText-2 (FINAL)")
    print("=" * 80)
    print("📚 USING REAL DATA ONLY - NO SYNTHETIC DATA")
    print("🔧 FIXED: Generation display, validation, and warnings")
    print("=" * 80)
    ...
    if not os.path.exists(data_config_path):
        ...
        data_config = {
            ...
            "batch_size": 4,
            "eval_batch_size": 8,
            "num_workers": 0,  # FIXED: Set to 0 to avoid tokenizer warnings
            "pin_memory": True,
        ...
        data_config["num_workers"] = 0  # FIXED: Override to prevent warnings
    ...
    # FIXED: Store corruptor in model for validation
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if batch_idx >= 20:  # REDUCED: Process 20 batches per epoch for faster testing
                break
        ...
        # FIXED: Validation with proper error handling
        try:
    ...
    # FIXED: Test generation on REAL data with better error handling
    ...
    try:
        ...
        with torch.no_grad():
            ...
            # FIXED: Better text decoding with error handling
            try:
            ...
            # FIXED: Better generated text decoding with error handling
            try:
                generated_text = data_module.train_dataset.decode(generated.squeeze())
                if generated_text.strip():  # Check if text is not empty
                    print(f"Generated text: {generated_text[:200]}...")
                else:
                    print("Generated text: [Empty or special tokens only]")
            ...
            # FIXED: Additional debugging info
...
if __name__ == "__main__":
    try:
        ...
        print("🔧 ALL ISSUES FIXED: Generation display, validation, warnings!")
    except Exception as e:
